...
function toCardData(c: RawCampaign): CampaignCardData {
  const totalRevenue = c.products.reduce(
    (sum, cp) => sum + Number(cp.campaignPrice) * Math.min(c.soldCount, c.totalStock ?? c.soldCount),
    0,
  );
  const avgPrice =
    c.products.length > 0
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const data = await getBrandCampaigns(brand.id, 'GONGGU');
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border border-gray-100 bg-white py-20 text-center shadow-sm">
      ...
      <p className="text-lg font-semibold text-gray-900">아직 캠페인이 없어요</p>
      <p className="mt-1 max-w-md text-sm text-gray-500">
        첫 공구 캠페인을 만들어서 크리에이터와 함께 판매를 시작해보세요
      </p>
      <button
        type="button"
        onClick={onCreate}
