...
export default function GlobalOrderDetailPage() {
  ...
  const getCurrentStep = () => {
    ...
    if (order.status === 'CONFIRMED') return PROGRESS_STEPS.length;
  ...
  return (
    <div className="pb-28">
      <div className="max-w-lg mx-auto px-4 pt-4">
        ...
        {/* Creator info box */}
        {order.creator && (
          <div className="bg-gradient-to-r from-pink-50 via-purple-50 to-pink-50 border border-pink-200 rounded-2xl p-4 mb-3">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="w-4 h-4 text-pink-500" />
              <h3 className="font-bold text-sm text-gray-900">이 주문을 추천한 크리에이터</h3>
            </div>
            <div className="flex items-center justify-between">
              ...
              >
                <div className="w-12 h-12 rounded-full overflow-hidden bg-white relative flex-shrink-0 ring-2 ring-pink-200">
                  {(order.creator.profileImage || order.creator.profileImageUrl) ? (
                    <Image
                      src={(order.creator.profileImage || order.creator.profileImageUrl)}
                      alt=""
                      fill
              ...
              <Link
                href={`/${locale}/${order.creator.username}`}
                className="px-4 py-2 bg-white text-pink-600 text-xs font-medium rounded-full border border-pink-300 hover:bg-pink-50 transition flex-shrink-0"
              >
                샵 방문
              </Link>
            </div>
        ...
        {order.brand && (
          <div className="bg-white rounded-2xl p-5 mb-3">
            ...
            <div className="mt-3 space-y-2">
              <Link
                href={`/${locale}/my/orders/${orderId}/inquiry`}
              ...
              {['DELIVERED', 'CONFIRMED'].includes(order.status) && (
                <div className="flex gap-2">
                  <Link
                    href={`/${locale}/my/orders/${orderId}/exchange`}
                  ...
                  <Link
                    href={`/${locale}/my/orders/${orderId}/refund`}
                    className="flex-1 flex items-center justify-center gap-1 h-10 border border-gray-200 rounded-xl text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                  >
                    반품 신청
                  </Link>
        ...
        <div className="max-w-lg mx-auto px-4 py-3 flex gap-3">
          {order.status === 'DELIVERED' && (
            <>
              ...
              <Link href={`/${locale}/my/orders/${orderId}/review/write`} className="flex-1">
          ...
          {order.status === 'CONFIRMED' && (
            <>
              ...
              {creatorUsername && (
                <Link href={`/${locale}/${creatorUsername}`} className="flex-1">
